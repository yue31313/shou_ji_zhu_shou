...
public class AppInfoManager {
	...
	public List<AppInfo> getUserApp() {
		...
		for (int i = 0; i < userPackageList.size(); i++) {
			aib = new AppInfo();
			aib.setIcon(userPackageList.get(i).applicationInfo
					.loadIcon(context.getPackageManager()));
			aib.setName(userPackageList.get(i).applicationInfo.loadLabel(
					context.getPackageManager()).toString());
			aib.setBaoName(userPackageList.get(i).applicationInfo.packageName);
			aib.setVersionNumber(String.valueOf(userPackageList.get(i).versionCode));
	...
	public List<AppInfo> getSystemApp() {
		...
		for (int i = 0; i < systemPackageList.size(); i++) {
			aib = new AppInfo();
			aib.setIcon(systemPackageList.get(i).applicationInfo
					.loadIcon(context.getPackageManager()));
			aib.setName(systemPackageList.get(i).applicationInfo.loadLabel(
					context.getPackageManager()).toString());
			aib.setBaoName(systemPackageList.get(i).applicationInfo.packageName);
			aib.setVersionNumber(String.valueOf(allPackageInfos.get(i).versionCode));
			...
			for (int j = 0; j < runningAppProcessInfos.size(); j++) {
				...
				if (allPackageInfos.get(i).packageName
					...
					aib.setName(allPackageInfos.get(i).applicationInfo
							.loadLabel(context.getPackageManager()).toString());
					// 包名
					aib.setBaoName(allPackageInfos.get(i).applicationInfo.packageName);

					// pid
					aib.setPid(runningAppProcessInfos.get(j).pid);
					aib.setServicen(getServiceNumber(aib.getPid()));// 暂时没能获取服务数量
					list.add(aib);
	...
	public int getServiceNumber(int pid) {
		...
		for (int i = 0; i < runningServiceInfos.size(); i++) {
			if (runningServiceInfos.get(i).pid == pid
					&& runningServiceInfos.get(i).flags == RunningServiceInfo.FLAG_STARTED) {
				number++;
			}
		}
	...
	public String getTotalMemorys() {
		...
		try {
			...
			total_memory = Integer.valueOf(arrayOfString[1]).intValue() * 1024;// 获得系统总内存，单位是KB，乘以1024转换为Byte

			localBufferedReader.close();
		} catch (IOException e) {
		}

		return Formatter.formatFileSize(context, total_memory);// Byte转换为KB或者MB，内存大小规格化
																// }
	}
	...
	public boolean killProcess(ArrayList<String> hs) {
	...
	//CPU名字
		...
		public String getProvidersName() {
		 ...
		 TelephonyManager getSystemService(String telephonyService) {
